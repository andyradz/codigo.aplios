...
/**
 * Kopiec binarny A binary heap is a heap data structure created using a binary tree. It can be seen
 * as a binary tree with two additional constraints:<br>
 * <br>
 * 1) The shape property: the tree is a complete binary tree; that is, all levels of the tree,
 * except possibly the last one (deepest) are fully filled, and, if the last level of the tree is
 * not complete, the nodes of that level are filled from left to right.<br>
 * <br>
 * 2) The heap property: each node is right than or equal to each of its children according to a
 * comparison predicate defined for the data structure.
 * <p>
 *
 * @param <T>
...
public interface BinaryHeap<T extends Comparable<T>> extends IHeap<T> {
	...
	public static class BinaryHeapArray<T extends Comparable<T>> implements BinaryHeap<T> {
		...
		protected void heapDown(final int index) {
			...
			final T right = (rightIndex != Integer.MIN_VALUE) && (rightIndex < this.size) ? this.array[rightIndex]
					: null;
			...
			private static <T extends Comparable<T>> String getString(final BinaryHeapArray<T> tree, final int index,
				...
				if (children != null) {
					for (int i = 0; i < (children.size() - 1); i++)
						builder
							.append(HeapPrinter
								.getString(tree, children.get(i), prefix + (isTail ? "    " : "│   "), false));
					if (children.size() >= 1)
						builder
							.append(HeapPrinter
								.getString(tree, children.get(children.size() - 1), prefix + (isTail ? "    " : "│   "),
										true));
				}
				...
				private static <T extends Comparable<T>> String getString(final Node<T> node, final String prefix,
					...
					if (children != null) {
						for (int i = 0; i < (children.size() - 1); i++)
							builder
								.append(HeapPrinter
									.getString(children.get(i), prefix + (isTail ? "    " : "│   "), false));
						if (children.size() >= 1)
							builder
								.append(HeapPrinter
									.getString(children.get(children.size() - 1), prefix + (isTail ? "    " : "│   "),
											true));
					}
