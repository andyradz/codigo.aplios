...
// assertEquals("Mismatched error code", WinError.ERROR_INVALID_HANDLE,
...
// char buffer[] = new char[WinBase.MAX_COMPUTERNAME_LENGTH + 1];
// lpnSize.setValue(buffer.length);
// assertTrue("Failed to retrieve expected computer name", Kernel32.INSTANCE.GetComputerName(buffer,
// lpnSize));
...
// assertNotNull("Failed (" + Kernel32.INSTANCE.GetLastError() + ") to get process ID=" + pid + "
// handle", h);
//
// try {
...
// assertTrue("Version high should be non-zero: 0x" +
// Integer.toHexString(version.getHigh().intValue()), version
// .getHigh().intValue() != 0);
// assertTrue("Version low should be >= 0: 0x" + Integer.toHexString(version.getLow().intValue()),
// version.getLow()
// .intValue() >= 0);
// }
//
...
// }
//
// HANDLE hFile = Kernel32.INSTANCE.CreateFile(tmp.getAbsolutePath(), WinNT.GENERIC_READ,
...
// try {
// assertTrue(Kernel32.INSTANCE.SetHandleInformation(hFile, WinBase.HANDLE_FLAG_PROTECT_FROM_CLOSE,
// 0));
// } finally {
// Kernel32Util.closeHandle(hFile);
// }
...
// assertTrue(Kernel32.INSTANCE.GetExitCodeProcess(Kernel32.INSTANCE.GetCurrentProcess(),
// lpExitCode));
...
// tmp.deleteOnExit();
//
// HANDLE hFile = Kernel32.INSTANCE.CreateFile(tmp.getAbsolutePath(), WinNT.GENERIC_ALL,
// WinNT.FILE_SHARE_READ,
// new WinBase.SECURITY_ATTRIBUTES(), WinNT.OPEN_EXISTING, WinNT.FILE_ATTRIBUTE_NORMAL, null);
// assertFalse(WinBase.INVALID_HANDLE_VALUE.equals(hFile));
//
...
// WinNT.OPEN_EXISTING,
// WinNT.FILE_ATTRIBUTE_DIRECTORY | WinNT.FILE_FLAG_BACKUP_SEMANTICS |
...
//
// try {
// SymbolicLinkReparseBuffer symLinkReparseBuffer = new
// SymbolicLinkReparseBuffer(folder.getFileName()
// .toString(),
...
// assertEquals(folder.getFileName().toString(),
// lpBuffer.u.symLinkReparseBuffer.getSubstituteName());
// } finally {
// Kernel32Util.closeHandle(hFile);
// }
// } finally {
...
//
// Kernel32.INSTANCE
// .MoveFileEx(source.getCanonicalPath(), destination.getCanonicalPath(), new
// DWORD(WinBase.MOVEFILE_REPLACE_EXISTING));
// assertTrue(destination.exists());
// assertFalse(source.exists());
// }
//
...
// WinBase.PROCESS_INFORMATION.ByReference processInformation = new
...
// File tmpFile =
// new File(Files.createTempFile(tmpDir, "testFindFirstFileExFindExInfoStandard",
// ".jna").toString());
//
// Memory p = new Memory(WIN32_FIND_DATA.sizeOf());
...
// File tmpFile = new File(Files.createTempFile(tmpDir, "testFindFirstFileExFindExInfoBasic",
// ".jna").toString());
//
// Memory p = new Memory(WIN32_FIND_DATA.sizeOf());
...
// tmp.deleteOnExit();
//
// HANDLE hFile = Kernel32.INSTANCE.CreateFile(tmp.getAbsolutePath(), WinNT.GENERIC_WRITE,
...
// if (false == Kernel32.INSTANCE.GetFileInformationByHandleEx(hFile, WinBase.FileStandardInfo, p,
// new DWORD(p
// .size()))) {
// fail("GetFileInformationByHandleEx failed with " + Kernel32.INSTANCE.GetLastError());
// }
...
// }
//
// public void testSetFileInformationByHandleFileBasicInfo() throws IOException,
...
//
// if (false == Kernel32.INSTANCE
// .SetFileInformationByHandle(hFile, WinBase.FileBasicInfo, fbi.getPointer(), new
// DWORD(FILE_BASIC_INFO
// .sizeOf()))) {
// fail("GetFileInformationByHandleEx failed with " + Kernel32.INSTANCE.GetLastError());
// }
//
...
// }
//
// public void testSetFileInformationByHandleFileDispositionInfo() throws IOException,
// InterruptedException {
...
// tmp.deleteOnExit();
//
// Kernel32.INSTANCE.SetFileAttributes(tmp.getCanonicalPath(), new
// DWORD(WinNT.FILE_ATTRIBUTE_HIDDEN));
...
// writer.close();
//
// assertEquals(123, Kernel32.INSTANCE.GetPrivateProfileInt("Section", "existingKey", 456,
// tmp.getCanonicalPath()));
// assertEquals(456, Kernel32.INSTANCE.GetPrivateProfileInt("Section", "missingKey", 456,
// tmp.getCanonicalPath()));
// }
//
...
// writer.close();
//
// assertTrue(Kernel32.INSTANCE.WritePrivateProfileString("Section", "existingKey", "DEF",
// tmp.getCanonicalPath()));
// assertTrue(Kernel32.INSTANCE.WritePrivateProfileString("Section", "addedKey", "GHI",
// tmp.getCanonicalPath()));
// assertTrue(Kernel32.INSTANCE.WritePrivateProfileString("Section", "removedKey", null,
// tmp.getCanonicalPath()));
//
...
// final DWORD len = Kernel32.INSTANCE.GetPrivateProfileSection("X", buffer, new
// DWORD(buffer.length), tmp
// .getCanonicalPath());
//
...
// final DWORD len = Kernel32.INSTANCE.GetPrivateProfileSectionNames(buffer, new
// DWORD(buffer.length), tmp
// .getCanonicalPath());
...
// SIZE_T bytesRead = Kernel32.INSTANCE.VirtualQueryEx(selfHandle, Pointer.NULL, mbi, new
// SIZE_T(mbi.size()));
...
// WinNT.GENERIC_READ | WinNT.GENERIC_WRITE, 0, null, WinNT.OPEN_EXISTING,
// WinNT.FILE_ATTRIBUTE_NORMAL,
// null);
//
...
// handleSerialPort = Kernel32.INSTANCE.CreateFile("\\\\.\\com1", WinNT.GENERIC_READ |
// WinNT.GENERIC_WRITE, 0,
...
// assertTrue("Session should be 1 or higher because of service isolation", pSessionId.getValue() >
// 0);
// }
//
...
// try {
// hModule = Kernel32.INSTANCE.LoadLibraryEx(new File(winDir, "explorer.exe").getAbsolutePath(),
// null,
...
// assertEquals("GetLastError should be set to 1813", WinError.ERROR_RESOURCE_TYPE_NOT_FOUND,
// Kernel32.INSTANCE
// .GetLastError());
// }
//
...
// assertEquals("GetLastError should be set to 0", WinError.ERROR_SUCCESS,
// Kernel32.INSTANCE.GetLastError());
...
// .CreateToolhelp32Snapshot(Tlhelp32.TH32CS_SNAPMODULE, new
// DWORD(Kernel32.INSTANCE.GetCurrentProcessId()));
// if (snapshot == null) {
...
// // check tests both
// assertTrue("The first module in the current process should be java.exe or javaw.exe",
// first.szModule()
// .startsWith("java"));
