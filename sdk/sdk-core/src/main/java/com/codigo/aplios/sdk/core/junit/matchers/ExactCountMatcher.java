 ...
 * @category testing
 */
public final class ExactCountMatcher
        extends TypeSafeDiagnosingMatcher<List<String>> {
    ...
    protected boolean matchesSafely(final List<String> items, final Description mismatchDescriptor) {
        ...
        if (!okay) {
            ...
            mismatchDescriptor.appendValueList(separator,
                    separator,
                    "",
                    items);
    ...
    public static Matcher<? super List<String>> hasNoLinesThat(final Matcher<String> stringMatcher) {
        return new ExactCountMatcher(stringMatcher,
                0);
    }
