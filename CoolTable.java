...
public class CoolTable extends JScrollPane {
	...
	public static void main(String[] args) {
		...
		final CoolTable coolTable = new CoolTable(
	...
	public CoolTable(TableModel model, int numFrozenColumns) {
		...
		final Action lockedTablePrevColumnCellAction = this.getAction(this.lockedTable, KeyEvent.VK_TAB,
				InputEvent.SHIFT_DOWN_MASK);
		final Action scrollTablePrevColumnCellAction = this.getAction(this.scrollTable, KeyEvent.VK_TAB,
				InputEvent.SHIFT_DOWN_MASK);

		this.setAction(this.lockedTable, "selectNextColumn", new LockedTableSelectNextColumnCellAction(
			lockedTableNextColumnCellAction));
		this.setAction(this.scrollTable, "selectNextColumn", new ScrollTableSelectNextColumnCellAction(
			scrollTableNextColumnCellAction));
		this.setAction(this.lockedTable, "selectPreviousColumn", new LockedTableSelectPreviousColumnCellAction(
			lockedTablePrevColumnCellAction));
		this.setAction(this.scrollTable, "selectPreviousColumn", new ScrollTableSelectPreviousColumnCellAction(
			scrollTablePrevColumnCellAction));

		this.setAction(this.lockedTable, "selectNextColumnCell", new LockedTableSelectNextColumnCellAction(
			lockedTableNextColumnCellAction));
		this.setAction(this.scrollTable, "selectNextColumnCell", new ScrollTableSelectNextColumnCellAction(
			scrollTableNextColumnCellAction));
		this.setAction(this.lockedTable, "selectPreviousColumnCell", new LockedTableSelectPreviousColumnCellAction(
			lockedTablePrevColumnCellAction));
		this.setAction(this.scrollTable, "selectPreviousColumnCell", new ScrollTableSelectPreviousColumnCellAction(
			scrollTablePrevColumnCellAction));
	...
	public class JScrollPaneAdjuster implements PropertyChangeListener, Serializable {
		...
		public JScrollPaneAdjuster(JScrollPane pane) {
			...
			this.pane = pane;
			this.x = new Adjuster(
				pane.getViewport(), pane.getColumnHeader(), Adjuster.X);
			this.y = new Adjuster(
				pane.getViewport(), pane.getRowHeader(), Adjuster.Y);
		...
		private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
			...
			this.x = new Adjuster(
				this.pane.getViewport(), this.pane.getColumnHeader(), Adjuster.X);
			this.y = new Adjuster(
				this.pane.getViewport(), this.pane.getRowHeader(), Adjuster.Y);
		}
		...
		private class Adjuster implements ChangeListener, Runnable {
			...
			public void run() {
				...
				if (this.type == Adjuster.X) {
					if (v.x != h.x)
						this.viewport.setViewPosition(new Point(
							h.x, v.y));
				}
				else if (v.y != h.y)
					this.viewport.setViewPosition(new Point(
	...
	private final class LockedTableSelectLastColumnCellAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			CoolTable.this.scrollTable.changeSelection(CoolTable.this.scrollTable.getSelectedRow(),
					CoolTable.this.scrollTable.getColumnCount() - 1, false, false);
		}
	}
	...
	private final class LockedTableSelectNextColumnCellAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (CoolTable.this.lockedTable.getSelectedColumn() == (CoolTable.this.lockedTable.getColumnCount() - 1)) {
				CoolTable.this.lockedTable.transferFocus();
				CoolTable.this.scrollTable.changeSelection(CoolTable.this.lockedTable.getSelectedRow(), 0, false,
						false);
			}
			else
	...
	private final class ScrollTableSelectNextColumnCellAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (CoolTable.this.scrollTable.getSelectedColumn() == (CoolTable.this.scrollTable.getColumnCount() - 1)) {
				CoolTable.this.scrollTable.transferFocusBackward();
				CoolTable.this.lockedTable.changeSelection(CoolTable.this.nextRow(CoolTable.this.scrollTable), 0, false,
						false);
				return;
			}
			else
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (CoolTable.this.scrollTable.getSelectedColumn() == 0) {
				...
				CoolTable.this.lockedTable.changeSelection(CoolTable.this.scrollTable.getSelectedRow(),
						CoolTable.this.lockedTable.getColumnCount() - 1, false, false);
				return;
			}
			else
