...
public class MockSecurityManager extends SecurityManager {
	...
	private static final Permission	setSecurityManagerPerm	= new RuntimePermission(
		"setSecurityManager");
	...
	/**
	 * Define the set of permissions to be granted to classes in the o.a.c.l package, but NOT to
	 * unit-test classes in o.a.c.l.security package.
	 */
	...
	/**
	 * This returns the number of times that a check of a permission failed due to stack-walking tracing
	 * up into untrusted code. Any non-zero value indicates a bug in JCL, ie a situation where code was
	 * not correctly wrapped in an AccessController block. The result of such a bug is that signing JCL
	 * is not sufficient to allow JCL to perform the operation; the caller would need to be signed too.
	 */
		...
		for (int i = 1; i < stack.length; ++i) {
			...
			if (cname.equals("java.security.AccessController")) {
				...
				if (this.permissions.implies(p))
					...
					throw new SecurityException(
						"Permission refused:" + p.getClass() + ":" + p);
				}
			}
			else {
