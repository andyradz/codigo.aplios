...
public class FileSystemModel2 extends AbstractTreeTableModel {
	 ...
	 */
	static Stack<MergeSort> sorters = new Stack<>();
	...
	protected static MergeSort getSizeSorter() {
		...
		synchronized (FileSystemModel2.sorters) {
			if (FileSystemModel2.sorters.isEmpty())
				return new SizeSorter();
			return FileSystemModel2.sorters.pop();
		}
	}
	...
	/**
	 * Creates a FileSystemModel2 with the root being <code>rootPath</code>. This does not load it, you
		...
		try {
			...
			case 1:
				if (fn.isTotalSizeValid())
					return Integer.valueOf((int) ((FileNode) node).totalSize());
				return null;
			case 2:
	...
	private static FileNode[] EMPTY_CHILDREN = new FileNode[0];
	...
	class FileNode {
		...
		protected void setChildren(final FileNode[] newChildren, final boolean generateEvent) {
			...
			if (generateEvent) {
				final FileNode[] path = this.getPath();
		...
		protected synchronized void childTotalSizeChanged(final FileNode child) {
			...
			if (this.totalSizeValid != child.isTotalSizeValid())
				if (this.totalSizeValid)
					...
					final FileNode[] children = this.getChildren();
	 ...
	 * children of the FileNode it is created with, and in its run method will reload all of that nodes
	 ...
	 * SwingUtilities.invokeLater is used so that events are generated in the event dispatching thread.
	 */
	...
	static class SizeSorter extends MergeSort {
		...
		public int compareElementsAt(final int beginLoc, final int endLoc) {
			...
			final long firstSize = ((FileNode) this.toSort[beginLoc]).totalSize();
			final long secondSize = ((FileNode) this.toSort[endLoc]).totalSize();
