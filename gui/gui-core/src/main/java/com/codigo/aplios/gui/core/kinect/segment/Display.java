...
public class Display
     ...
     *
     * @post: m_displayDigits is intialized to an array of four 10s, m_display colon is initialized to false, and
     * m_displayMsg is initialized to an empty String.
     */
    public Display() {
    ...
    public Dimension getPreferredSize() {
        return new Dimension(1500,
                400);
    }

    /**
     * Draws the clock display based off of the display information variables. This page in the Java documentation was
     * referenced in creation of this method: https://docs.oracle.com/javase/tutorial/2d/geometry/arbitrary.html
     *
     * @param g
     *          the Graphics object to paint on
     * @post the display will be drawn representing the current state of m_displayDigits, m_displayColon, and
     * m_displayMsg
     */
    @Override
    public void paintComponent(final Graphics g) {
        ...
        for (int digit = 0; digit < 6; digit++) {
            final boolean[][] sevenSeg = this.digitToArray(this.m_displayDigits[digit]);
            ...
            for (int xIter = 0; xIter < 2; xIter++) {
                ...
                for (int yIter = 0; yIter < 2; yIter++)
                    if (sevenSeg[1][(xIter * 2) + yIter]) {
                        ...
                        final GeneralPath segment = new GeneralPath(Path2D.WIND_EVEN_ODD,
                                xVertSeg.length);
    ...
    /**
     * Associates a given digit with it's 7 segment display representation as a 2d array of booleans. The array is
     * organized as the first three horizontal segments descending and then the vertical segments moving down first then
     * to the right. Anything other than a digit 0-9 will result in the function returning a blank representation (use
     * this to display nothing).
     *
     * @param digit
