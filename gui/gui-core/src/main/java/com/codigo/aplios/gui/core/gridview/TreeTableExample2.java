...
public class TreeTableExample2 {
	...
	/**
	 * Creates and returns the instanceof JTreeTable that will be used. This also creates, but does not
	...
	protected JMenuBar createMenuBar() {
		...
		for (final LookAndFeelInfo laf : lafs) {
			...
			rb.addItemListener(ae -> {
				...
				if (rb2.isSelected()) {
					final UIManager.LookAndFeelInfo info = (UIManager.LookAndFeelInfo) rb2.getClientProperty("UIKey");
					try {
	...
	/**
	 * Reloader is the ActionListener used in the Timer. In response to the timer updating it will reset
	...
	class Reloader implements ActionListener, TreeExpansionListener {
		...
		protected void generateChangeEvent(final int row) {
			...
			if (row != -1) {
				final AbstractTableModel tModel = (AbstractTableModel) TreeTableExample2.this.treeTable.getModel();
	...
	class IndicatorRenderer extends DefaultTableCellRenderer {
		...
		private static final long serialVersionUID = 4267584043544321422L;
