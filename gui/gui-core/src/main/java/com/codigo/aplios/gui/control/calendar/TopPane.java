...
import com.codigo.aplios.sdk.controls.calendar.FXCalendarControls.CalendarToggleButton;
import com.codigo.aplios.sdk.controls.calendar.FXCalendarControls.NormalButton;
import com.codigo.aplios.sdk.controls.calendar.FXCalendarControls.YearNavigatorArrowButton;
...
public class TopPane extends Group {
	...
	private void configureMonthPane(HBox hb) {
		...
		monthPane.setPrefWidth((datePicker.getBounds()
				.getWidth() - 1) / 2);
		monthPane.setPrefHeight(164);
		monthPane.getStyleClass()
	...
	private void generateMonthButtons() {
		...
		String[] months = datePicker.getFXCalendarUtility()
				.getShortMonths(datePicker.getLocale());
		int evenValue = 0;
		int oddValue = 6;
		...
		for (int i = 0; i < 12; i++) {
			...
			final CalendarToggleButton btn = new FXCalendarControls().new CalendarToggleButton(months[pos],
					pos);
	...
	private void configureYearPane(HBox hb) {
		...
		yearPane.setPrefWidth(datePicker.getBounds()
				.getWidth() / 2);
		yearPane.setPrefHeight(164);
		yearPane.getStyleClass()
		...
		prevBtn = new FXCalendarControls().new YearNavigatorArrowButton(Side.LEFT,
				datePicker.getBaseColor());
		YearNavigatorArrowButton nextBtn = new FXCalendarControls().new YearNavigatorArrowButton(Side.RIGHT,
				datePicker.getBaseColor());
	...
	private void configureFooter() {
		...
		footerPane.setPrefWidth(datePicker.getBounds()
				.getWidth());
		footerPane.setPrefHeight(32);
		footerPane.getStyleClass()
