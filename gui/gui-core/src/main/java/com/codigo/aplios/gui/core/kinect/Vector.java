package com.codigo.aplios.gui.core.kinect;

/**
 * Vector holds values representing a vector in Euclidean 2-space. For convenience, this class exposes the Vector
 * components as public, mutable fields. It should remain package-private.
 *
 * @see Particle
 * @see Ensemble
 * @author John B. Matthews
 */
class Vector
        extends Object {

    /**
     * The Vector's x component.
     */
    public double x;

    /**
     * The Vector's y component.
     */
    public double y;

    /**
     * Construct a null Vector, <0, 0> by default.
     */
    public Vector() {
    }

    /**
     * Construct a new Vector from two doubles.
     *
     * @param x
     *          the x component
     * @param y
     *          the y component
     */
    public Vector(final double x, final double y) {
        this.x = x;
        this.y = y;
    }

    /**
     * Construct a new Vector from two integers.
     *
     * @param x
     *          the x component
     * @param y
     *          the y component
     */
    public Vector(final int x, final int y) {
        this.x = x;
        this.y = y;
    }

    /**
     * Construct a new Vector from an exising one.
     *
     * @param v
     *          the source Vector
     */
    public Vector(final Vector v) {
        this.x = v.x;
        this.y = v.y;
    }

    /**
     * Set the components of this Vector.
     *
     * @param x
     *          the x component
     * @param y
     *          the y component
     * @return this Vector.
     */
    public Vector set(final double x, final double y) {
        this.x = x;
        this.y = y;
        return this;
    }

    /**
     * Set the components of this Vector to those of v.
     *
     * @param v
     *          the source Vector
     * @return this Vector.
     */
    public Vector set(final Vector v) {
        this.x = v.x;
        this.y = v.y;
        return this;
    }

    /**
     * Return the scalar norm (length) of this Vector.
     *
     * @return the norm of this Vector
     */
    public double norm() {
        return Math.hypot(this.x, this.y);
    }

    /**
     * Add the given Vector to this Vector; return this Vector.
     *
     * @param v
     *          the given Vector
     * @return the sum
     */
    public Vector add(final Vector v) {
        this.x += v.x;
        this.y += v.y;
        return this;
    }

    /**
     * Subtract the given Vector from this Vector; return this Vector.
     *
     * @param v
     *          the given Vector
     * @return the difference
     */
    public Vector subtract(final Vector v) {
        this.x -= v.x;
        this.y -= v.y;
        return this;
    }

    /**
     * Multiply the given Vector by this Vector; return the scalar product.
     *
     * @param v
     *          the given Vector
     * @return the scalar (dot) product
     */
    public double dot(final Vector v) {
        return (this.x * v.x) + (this.y * v.y);
    }

    /**
     * Scale this Vector by the given scale factor.
     *
     * @param s
     *          the scale factor
     * @return the this Vector, scaled by s
     */
    public Vector scale(final double s) {
        this.x *= s;
        this.y *= s;
        return this;
    }

    /**
     * Scale this Vector by 1 / norm(); return this Vector. The result is a unit Vector parallel to the original. This
     * is equivalent to this.scale(1 / this.norm()), with a check for division by zero.
     *
     * @return the this Vector, scaled to unit length
     */
    public Vector unitVector() {
        final double d = this.norm();
        if (d != 0) {
            this.x /= d;
            this.y /= d;
        }
        return this;
    }

    /**
     * Return this Vector's String representation.
     */
    @Override
    public String toString() {
        return "<" + this.x + ", " + this.y + ">";
    }

}
