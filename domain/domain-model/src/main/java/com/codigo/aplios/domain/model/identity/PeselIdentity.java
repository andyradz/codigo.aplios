...
public final class PeselIdentity {
	...
	/**
	 * Podstawowy konstruktor obiektu klasy <code>PeselIdentity</code>
	 *
	 * @param pesel
	 ...
	 * @throws Exception
	 */
	public PeselIdentity(final String pesel) throws NullPointerException, IllegalArgumentException, Exception {
		...
		if (PeselIdentity.PESEL_LENGTH != pesel.length())
			throw new IllegalArgumentException(
				"Numer pesel ma nieprawidłową długość!");

		if (!pesel.matches(PeselIdentity.PESEL_REGEX))
			throw new Exception(
				"Numer pesel może składać się wyłącznie z cyfr!");
	...
	public static void main(final String[] args) throws NullPointerException, IllegalArgumentException, Exception {

		PeselIdentity id = new PeselIdentity(
			"44051401358");
		Gender gender = id.getGender();
		id = new PeselIdentity(
			"49040501580");
		gender = id.getGender();
		...
		id = new PeselIdentity(
			"79062601652");
		gender = id.getGender();
		final String date = id.getBirthOfYear() + id.getMonthOfBirth() + id.getDayOfBirth();
