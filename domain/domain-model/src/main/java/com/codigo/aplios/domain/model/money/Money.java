...
public final class Money implements Comparable<Money>, Serializable {
	 ...
	 *
	 * <em>Your application must call this method upon startup</em>. This method should usually be
	 ...
	 *
	 * @param amount
	 *        is required, can be positive or negative. The number of decimals in the amount cannot
	 *        <em>exceed</em> the maximum number of decimals for the given {@link Currency}. It's
	 *        possible to create a <tt>Money</tt> object in terms of 'thousands of dollars', for
	...
	public Money plus(final Money money) {
		...
		return new Money(
			this.mAmount.add(money.mAmount), this.mCurrency, this.mRounding);
	}
	...
	public Money minus(final Money money) {
		...
		checkCurrenciesMatch(money);
		return new Money(
			this.mAmount.subtract(money.mAmount), this.mCurrency, this.mRounding);
	}

	/**
	 * Sum a collection of <tt>Money</tt> objects. Currencies must match. You are encouraged to use
	 * database summary functions whenever possible, instead of this method.
	 *
	 * @param moneys
	 *        collection of <tt>Money</tt> objects, all of the same currency. If the collection is
	 *        empty, then a zero value is returned.
	 * @param currencyIfEmpty
	 *        is used only when <tt>aMoneys</tt> is empty; that way, this method can return a zero
	 *        amount in the desired currency.
	 */
	public static Money sum(final Collection<Money> moneys, final Currency currencyIfEmpty) {

		Money sum = new Money(
			BigDecimal.ZERO, currencyIfEmpty);
	...
	/**
	 * Procedura wykonuje sprawdzenie logiczne czy przekazana wartość pieniężna jest większa od wartości
	 * pieniężnej bieżącego obiektu.
	 *
	 * @param money
	 ...
	 *
	 * <P>
	 * Return <tt>true</tt> only if 'this' amount is greater than or equal to 'that' amount. Currencies
	 * must match.
	 */
	 ...
	 *
	 * <P>
	 * Return <tt>true</tt> only if 'this' amount is less than or equal to 'that' amount. Currencies
	 * must match.
	 */
	...
	public Money times(final int factor) {
		...
		final BigDecimal newFactor = new BigDecimal(
			factor);
		final BigDecimal newAmount = this.mAmount.multiply(newFactor);
		return new Money(
			newAmount, this.mCurrency, this.mRounding);
	}
	...
	public String toString() {
		...
		return this.mAmount.toPlainString() + " " + this.mCurrency.getSymbol();
	}

	/**
	 * Like {@link BigDecimal#equals(java.lang.Object)}, this <tt>equals</tt> method is also sensitive
	 * to scale.
	 *
	 * For example, <tt>10</tt> is <em>not</em> equal to <tt>10.00</tt> The {@link #eq(Money)} method,
	 * on the other hand, is <em>not</em> sensitive to scale.
	 */
	@Override
	...
	private void readObject(final ObjectInputStream inputStream) throws ClassNotFoundException, IOException {
		...
		this.mAmount = new BigDecimal(
			this.mAmount.toPlainString());
	...
	private void validateState() {
		...
		if (this.mAmount.scale() > getNumDecimalsForCurrency())
			throw new IllegalArgumentException(
				"Number of decimals is " + this.mAmount.scale() + ", but currency only takes "
	...
	private void checkCurrenciesMatch(final Money money) {
		...
		if (!this.mCurrency.equals(money.getCurrency()))
			throw new MismatchedCurrencyException(
				money.getCurrency() + " doesn't match the expected currency : " + this.mCurrency);
	}
