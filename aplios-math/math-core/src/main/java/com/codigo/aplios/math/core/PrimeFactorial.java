...
public class PrimeFactorial {
	...
	public static void main(final String[] args) {
		...
		for (long idx = 0; idx <= 1_000; idx++) {
			final Iterator<Long> iter = PrimeFactorial.dissect(idx);
	...
	/**
	 * Metoda realizuje rozkład liczby naturalnej na czynniki pierwsze. Zastosowano matematyczny
	 * algorytm rozkładu na czynniki pierwsze.
	 *
	 * @param number
	 *        Wartość numeryczna liczby naturalnej.
	 */
	public static Iterator<Long> dissect(long number) {
		...
		final long orginalValue = number;

		final double value = Math.floor(Math.sqrt(number));
